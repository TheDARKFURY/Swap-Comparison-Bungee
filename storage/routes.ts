      ...
      for (const provider of providers) {
        if (provider.name === "LiFi") {
          ...
          for (const usdAmount of uniqueAmounts) {
            ...
            // LiFi with 0.25% fee adjustment
            const adjustedAmount = usdAmount + (usdAmount * 0.0025); // Add 0.25%
            console.log(`\n=== Processing ${amountKey} + 0.25% fee (${adjustedAmount.toFixed(2)}) for ${provider.name} with fee adjustment ===`);
            
            quotes["lifi_with_fee"][amountKey] = await getLiFiQuote(
              fromChain, fromToken, toChain, toToken, adjustedAmount, provider.apiKey, fromTokenPrice, toTokenPrice
            );
          }
    ...
    if (result.manualRoutes && result.manualRoutes.length > 0) {
      ...
      for (let i = 0; i < result.manualRoutes.length; i++) {
        ...
        if (route.output && route.output.amount) {
          ...
          // Extract provider name from routeDetails.name (primary) with fallbacks
          let providerName = 'Unknown';
          
          // Primary source: routeDetails.name (as seen in console logs)
          if (route.routeDetails && route.routeDetails.name) {
            providerName = route.routeDetails.name;
            console.log(`✅ [PROVIDER FOUND] routeDetails.name: ${providerName}`);
          } else if (route.usedBridgeNames && Array.isArray(route.usedBridgeNames) && route.usedBridgeNames.length > 0) {
            providerName = route.usedBridgeNames[0];
            console.log(`✅ [PROVIDER FOUND] usedBridgeNames[0]: ${providerName}`);
          } else if (route.bridgeName) {
            providerName = route.bridgeName;
            console.log(`✅ [PROVIDER FOUND] bridgeName: ${providerName}`);
          } else if (route.bridgeId) {
            providerName = route.bridgeId;
            console.log(`✅ [PROVIDER FOUND] bridgeId: ${providerName}`);
          } else if (route.protocol?.displayName) {
            providerName = route.protocol.displayName;
            console.log(`✅ [PROVIDER FOUND] protocol.displayName: ${providerName}`);
          } else if (route.protocol?.name) {
            providerName = route.protocol.name;
            console.log(`✅ [PROVIDER FOUND] protocol.name: ${providerName}`);
          } else if (route.protocolName) {
            providerName = route.protocolName;
            console.log(`✅ [PROVIDER FOUND] protocolName: ${providerName}`);
          } else if (route.steps && Array.isArray(route.steps) && route.steps.length > 0) {
            const firstStep = route.steps[0];
            if (firstStep.protocol?.displayName) {
              providerName = firstStep.protocol.displayName;
              console.log(`✅ [PROVIDER FOUND] steps[0].protocol.displayName: ${providerName}`);
            } else if (firstStep.protocol?.name) {
              providerName = firstStep.protocol.name;
              console.log(`✅ [PROVIDER FOUND] steps[0].protocol.name: ${providerName}`);
            } else if (firstStep.protocolName) {
              providerName = firstStep.protocolName;
              console.log(`✅ [PROVIDER FOUND] steps[0].protocolName: ${providerName}`);
            } else if (firstStep.toolDetails?.name) {
              providerName = firstStep.toolDetails.name;
              console.log(`✅ [PROVIDER FOUND] steps[0].toolDetails.name: ${providerName}`);
            } else if (firstStep.tool?.name) {
              providerName = firstStep.tool.name;
              console.log(`✅ [PROVIDER FOUND] steps[0].tool.name: ${providerName}`);
            } else if (firstStep.bridgeName) {
              providerName = firstStep.bridgeName;
              console.log(`✅ [PROVIDER FOUND] steps[0].bridgeName: ${providerName}`);
            } else {
              console.log(`❌ [PROVIDER NOT FOUND] steps[0] keys:`, Object.keys(firstStep));
            }
          } else if (route.toolDetails?.name) {
            providerName = route.toolDetails.name;
            console.log(`✅ [PROVIDER FOUND] toolDetails.name: ${providerName}`);
          } else if (route.tool?.name) {
            providerName = route.tool.name;
            console.log(`✅ [PROVIDER FOUND] tool.name: ${providerName}`);
          } else {
            console.log(`❌ [PROVIDER NOT FOUND] Available route keys:`, Object.keys(route));
          }
          
          console.log(`   Route ${i+1}: ${(outputAmount / Math.pow(10, toTokenDecimals)).toFixed(4)} tokens via ${providerName}`);
      ...
      if (bestManualRoute) {
        ...
        // Extract the actual provider name using routeDetails.name (primary) with fallbacks
        let displayProviderName = "Bungee Manual";
        
        if (bestManualRoute.routeDetails && bestManualRoute.routeDetails.name) {
          displayProviderName = bestManualRoute.routeDetails.name;
        } else if (bestManualRoute.usedBridgeNames && Array.isArray(bestManualRoute.usedBridgeNames) && bestManualRoute.usedBridgeNames.length > 0) {
          displayProviderName = bestManualRoute.usedBridgeNames[0];
        } else if (bestManualRoute.bridgeName) {
          displayProviderName = bestManualRoute.bridgeName;
        } else if (bestManualRoute.bridgeId) {
          displayProviderName = bestManualRoute.bridgeId;
        } else if (bestManualRoute.protocol?.displayName) {
          displayProviderName = bestManualRoute.protocol.displayName;
        } else if (bestManualRoute.protocol?.name) {
          displayProviderName = bestManualRoute.protocol.name;
        } else if (bestManualRoute.protocolName) {
          displayProviderName = bestManualRoute.protocolName;
        } else if (bestManualRoute.steps && Array.isArray(bestManualRoute.steps) && bestManualRoute.steps.length > 0) {
          const firstStep = bestManualRoute.steps[0];
          if (firstStep.protocol?.displayName) {
            displayProviderName = firstStep.protocol.displayName;
          } else if (firstStep.protocol?.name) {
            displayProviderName = firstStep.protocol.name;
          } else if (firstStep.protocolName) {
            displayProviderName = firstStep.protocolName;
          } else if (firstStep.toolDetails?.name) {
            displayProviderName = firstStep.toolDetails.name;
          } else if (firstStep.tool?.name) {
            displayProviderName = firstStep.tool.name;
          } else if (firstStep.bridgeName) {
            displayProviderName = firstStep.bridgeName;
          }
        } else if (bestManualRoute.toolDetails?.name) {
          displayProviderName = bestManualRoute.toolDetails.name;
        } else if (bestManualRoute.tool?.name) {
          displayProviderName = bestManualRoute.tool.name;
        }
        
        console.log(`🎯 [BUNGEE FINAL PROVIDER] Selected: "${displayProviderName}" for best manual route`);
        console.log(`📋 [BUNGEE BEST ROUTE KEYS]`, Object.keys(bestManualRoute));
